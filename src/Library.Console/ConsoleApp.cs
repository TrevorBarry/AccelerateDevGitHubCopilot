...
public class ConsoleApp
{
    ...
    static void WriteInputOptions(CommonActions options)
    {
        ...
        {
            Console.WriteLine(" - \"b\" to search for a book by title and check loan availability");
        }
    ...
    async Task SearchBooks()
    {
        ...
        var book = _jsonData.Books!.FirstOrDefault(b => b.Title.Equals(title, StringComparison.OrdinalIgnoreCase));
        ...
        var bookItemIds = _jsonData.BookItems!
            .Where(bi => bi.BookId == book.Id)
            .Select(bi => bi.Id)
            .ToHashSet();

        var activeLoan = _jsonData.Loans!
            .FirstOrDefault(l => bookItemIds.Contains(l.BookItemId) && l.ReturnDate == null);
